...
5. **硬性排除**：以下情况移到 ``excluded``，不进入 top5：
   - 公司明确为专利权人自身或其关联子公司；
   - 没有任何明确公司 / 产品 / 公开证据 URL；
   - 任一**必要技术特征**判定为"明确不满足"。
   - 若输入候选已经给出明确的产品上市/发布/量产日期，只有该日期晚于专利申请日时才可保留；若该日期不晚于专利申请日，应排除；若日期无法确定，不要因日期原因排除。
   - 注意：缺少尺寸、缺少能量、缺少型号细节、证据链不完整，只能判为"证据不足"或"可能满足"并写入 remaining_gaps；这不是硬性排除理由。证据不足只说明当前 Agent 尚未找到证据，不能说明公开证据一定不存在。
...
7. **Top5 选择**：按最终分数排序，取前 5 个。只要候选没有触发硬性排除，即使证据不足，也应保留为低分 / 待核查候选进入 top5；不要因为证据缺口明显就把 top5 缩到少于 5 个。
8. **人工复查列表**：把"证据缺口明显，值得人工继续核查"的候选，写入 ``needs_manual_review``，注明缺口与建议检索方向。进入 top5 的候选也可以同时出现在人工复查列表中。

【产品粒度要求】
- 最终 top5 的 ``product`` 应尽量指向已被证据支撑的具体型号 / 产品对象，而不是过宽的产品系列。
- 如果证据只证明某个具体规格（例如某个 Ah 型号、某个 L600 级型号），不要把该具体规格的尺寸参数泛化成整个 L400/L500/L600 系列都满足。
- 系列名、同系列型号、市场名称可以放入 ``aliases`` 或 ``remaining_gaps``，并说明哪些只是相关系列 / 待核查线索。
