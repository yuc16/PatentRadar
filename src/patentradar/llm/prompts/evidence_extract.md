...
## "可能满足" / "明确满足" 严谨推理规则（与 [`full_claim_chart.md`](./full_claim_chart.md) 同步——任一文件改动需两边同步）
...
## ⛔ URL 入 `evidence[]` 前必须验活（硬要求）

代码端已经按候选的 `source_urls` 跑过 fetch，结果在 `fetched_pages` 里。**LLM 选择哪些 URL 写进 `evidence[].url` 之前**，必须基于 `fetched_pages` 内容做验活——下列任一情况的 URL **一律丢弃**，不允许只看搜索结果摘要 / 标题就强行入池：

- **`fetched_pages` 里该 URL 的正文为空 / 仅几十字符** → fetch 失败或被屏蔽，丢弃
- **正文是反爬墙登录页 / 验证码页 / 付费墙摘要**（如只看到 "Please log in" / "请登录后查看" / 营销报告售卖页只列目录不给内容） → 丢弃
- **正文跟本候选完全无关**（如搜到的是其它品牌产品、行业概览文章里只一笔带过本候选名）→ 丢弃
- **URL 在 `fetched_pages` 里缺失**（代码端 fetch 时已经 404 / 5xx / 跨 host 跳转到首页等被丢了）→ **不要凭搜索摘要补回**，直接放弃这条 URL

**原因**：上一轮跑出来发现某些候选的 evidence URL（`cn.gii.tw/report/...`）实际是 301 死链跳到首页，律师点进去看不到任何竞品证据。这种死链 / 空壳页**不允许**进最终报告。

**实操**：写每条 `evidence[i].snippet` 时必须**引用 `fetched_pages` 里实际存在的字符串**（哪怕只是几个关键词），如果 snippet 是凭空写的——说明这条 URL 没通过验活，**必须丢弃**。
