...
```
还缺：<具体技术维度，写清"已有公开证据为何不足以证明本特征">
可去：<来源类型，逗号分隔，2-3 项>，例如 产品官方规格书 / 拆解视频 / 学术论文 / 行业评测站 / 公司专利文献 / 技术白皮书
下一步建议：<明确建议去哪里（可以是XX网站等）找什么>
```

**实例**（针对 BEIJING-X7 2020款 1.5TD DCT 致领版，C1-F6 「智能钥匙模块还包括设置在车辆后备箱、可探测钥匙感应单元的探测天线」）：

```
还缺：原始电路图页面确认功能、安装位置和连接关系。（目前仅为目录级证据：畅易维修目录列“8.4.3 行李箱天线”“8.4.5 后保险杠低频天线”。）。
可去：BEIJING-X7 2020款 1.5TD DCT 致领版的维修手册，BEIJING-X7 2020款 1.5TD DCT 致领版的维修手册的电路原理图
下一步建议：建议去https://m.asemanshop.com（畅易汽车网）或https://www.qixiu88.com（汽修巴巴），找该车型的维修资料
```

**写作要求**：
- "还缺"要具体到**技术维度**，不能笼统写"证据不足"
- "可去"必须是**真实来源类型**，不能凭空写"网络搜索"
- "下一步建议"必须是**给用户的明确建议**，不能笼统，如果建议内含url，必须是真实的，不能瞎编造
- 不要超过 3 行；每行不超过 80 字
