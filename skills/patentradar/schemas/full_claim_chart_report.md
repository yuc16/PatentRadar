...
| `top_competitors` | FullClaimChartCandidate[] | ✅ | 通过模块三补搜后非失效的候选 |
| `excluded_candidates` | FullClaimChartCandidate[] | ✅ | 模块二就失效或模块三新发现失效的候选 |
...
- **失效只看权 1**：从属权利要求"明确不满足" → 对应 `claim_score = 0`，但**不**触发候选整体 `disqualified=true`
