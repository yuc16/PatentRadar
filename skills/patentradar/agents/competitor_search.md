...
## 你的能力
- Web 搜索（多语言 query 自由拼）
- 抓 HTML / PDF（含 vision 看图）
- 不限 query 次数 / 不限看图数量 / 不限上下文长度——**搜索得越全越好**
...
每个候选公司多角度搜：规格书、拆解视频、第三方评测、官方发布、专利文献。

#### 跑搜索 + 抓 page

执行搜索 → 跟随高相关链接抓 page 原文（HTML + 必要时 PDF）。**遇到有图的页面（产品详情页/拆解文章/规格 PDF）就看图**——图里常有尺寸标注、内部结构、装配关系等文字证据没的。
...
- `product_version`: **产品介绍**（1-2 句自然语言，说明该产品的关键技术参数/规格/工艺；**不参与去重**）
- `market`: 市场或应用场景简短描述
...
- 看图（产品详情页/规格书/拆解照/尺寸标注图），尤其是文字里没的数值证据
- 不限 query 次数和看图数量——**搜得越全证据越扎实**
...
**严格不得自我宽松**：
- 只看到公司层面信息（"比亚迪是电池龙头"）→ 证据不足，不是可能满足
- 看到产品名但没具体参数 → 证据不足
...
- 完全找不到上市日期 → `launch_date = "未明确"`，**保留候选**（不视作失格）
  ...
  "top_competitors": [
    {
      "candidate": {
        ...
        "product_version": "第二代 3.2V 196Ah；21.5×574×118mm；627.2Wh",
